...
const IBtn = ({ onClick, title, emoji, style = {} }) => (
  <button title={title} onClick={onClick} style={{ background: "transparent", border: "none", cursor: "pointer", padding: "5px 6px", borderRadius: 6, fontSize: 14, flexShrink: 0, ...style }}>{emoji}</button>
);
...
export default function App() {
  ...
  const addGear = async () => { if (!ngItem.trim()) return; await addDoc(collection(db, "gear"), { item: ngItem, assignedTo: ngTo, addedBy: who, packed: false }); setNgItem(""); setAddGearOpen(false); };
  ...
  // ── Meal ops ──────────────────────────────────────────────────────
  const addMeal = async () => { if (!nmName.trim()) return; await addDoc(collection(db, "meals"), { name: nmName, day: nmDay, meal: nmType, chef: who }); setNmName(""); setAddMealOpen(false); };
  ...
  const addGroc = async () => { if (!ggItem.trim()) return; await addDoc(collection(db, "groceries"), { item: ggItem, qty: ggQty, category: ggCat, forMeal: ggMeal, addedBy: who, bought: false }); setGgItem(""); setGgQty(""); setGgMeal(""); setAddGrocOpen(false); };
  ...
  const toggleVote = (id, votedBy, votes) => {
    ...
    updateDoc(doc(db, "activities", id), {
      votes: voted ? votes - 1 : votes + 1,
      votedBy: voted ? votedBy.filter(m => m !== who) : [...(votedBy || []), who],
    });
  };
    ...
    <div style={{ minHeight: "100vh", background: "#f5f0e8", fontFamily: "'DM Sans',sans-serif", color: "#2d2a24", paddingBottom: 60 }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .fchip{background:#fff;border:1.5px solid #d4c9b8;border-radius:20px;padding:5px 12px;font-size:11px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;color:#6a6058;transition:all 0.15s}
        .fchip:hover,.fchip.active{border-color:#3a7a4a;color:#3a7a4a}
        .fchip.active{background:#e8f4ec}
      ...
      <div style={{ background: "linear-gradient(135deg,#3a7a4a,#2d6038)", padding: "34px 22px 28px", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 4 }}>
          ...
          <div className="live"><div className="live-dot" /><span style={{ fontSize: 10, color: "rgba(255,255,255,0.8)" }}>Live</span></div>
        </div>
      ...
      <div style={{ padding: "16px 16px 0" }}>
        ...
        {!loading && tab === "Overview" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
            {[
              ...
              <div key={s.label} style={{ background: s.bg, border: `1px solid ${s.color}22`, borderRadius: 13, padding: "13px 16px", display: "flex", alignItems: "center", gap: 13, boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
                <span style={{ fontSize: 24 }}>{s.icon}</span>
                <div>
                  <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 21, fontWeight: 700, color: s.color }}>{s.value}</div>
                  <div style={{ fontSize: 11, color: "#6a6058", fontWeight: 500 }}>{s.label}</div>
                </div>
              </div>
            ))}
        ...
        {!loading && tab === "Gear" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              ...
              <button className="abtn" onClick={() => { setAddGearOpen(v => !v); setEgId(null); }}>+ Add Item</button>
            </div>
            {addGearOpen && (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {gear.map(g => { const mine = g.addedBy === who || g.assignedTo === who; const editing = egId === g.id; return (
                <div key={g.id}>
                  <div className="row" style={rowBase(editing, g.packed ? "1px solid #7ab87a" : undefined)}>
                    ...
                    <span style={{ flex: 1, fontSize: 13, color: g.packed ? "#a09080" : "#2d2a24", textDecoration: g.packed ? "line-through" : "none" }}>{g.item}</span>
                    {g.addedBy !== g.assignedTo && <span style={{ fontSize: 10, color: "#b0a090" }}>by {g.addedBy}</span>}
                    <Avatar name={g.assignedTo} size={22} />
                    {mine && <><IBtn emoji="✏️" title="Edit" onClick={() => editing ? setEgId(null) : (setEg({ item: g.item, assignedTo: g.assignedTo }), setEgId(g.id))} /><IBtn emoji="🗑" title="Delete" onClick={() => delGear(g.id)} /></>}
                  </div>
                  {editing && <EditDrawer>
            ...
            <p style={{ marginTop: 10, fontSize: 11, color: "#b0a090", textAlign: "center" }}>✏️ edit · 🗑 delete · checkbox to mark packed</p>
          </div>
        )}
            ...
            {DAYS.map(day => { const dm = meals.filter(m => m.day === day); return (
              <div key={day} style={{ marginBottom: 18 }}>
                ...
                {dm.length === 0
                  ...
                  : dm.map(m => { const mc = { Breakfast: "#c45e38", Lunch: "#a07820", Dinner: "#2e7d4f", Snacks: "#8e44ad" }[m.meal] || "#2e7d4f"; const mbg = { Breakfast: "#fbe9e2", Lunch: "#faf3e0", Dinner: "#e2f2e9", Snacks: "#f0e8f8" }[m.meal] || "#e2f2e9"; const mi = { Breakfast: "🌅", Lunch: "☀️", Dinner: "🔥", Snacks: "🍎" }[m.meal] || "🍴"; const mine = m.chef === who; const editing = emId === m.id; return (
                    <div key={m.id} style={{ marginBottom: 5 }}>
                      <div className="row" style={rowBase(editing)}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                          <Avatar name={m.chef} size={26} /><span style={{ fontSize: 9, color: "#9a8a7a" }}>Chef</span>
                        </div>
                        {mine && <><IBtn emoji="✏️" title="Edit" onClick={() => editing ? setEmId(null) : (setEm({ name: m.name, day: m.day, meal: m.meal, chef: m.chef }), setEmId(m.id))} /><IBtn emoji="🗑" title="Delete" onClick={() => delMeal(m.id)} /></>}
                      </div>
                      {editing && <EditDrawer>
        ...
        {!loading && tab === "Groceries" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 17 }}>Groceries 🛒 <span style={{ fontSize: 12, color: "#9a8a7a", fontFamily: "'DM Sans',sans-serif" }}>({bought}/{groceries.length} bought)</span></span>
            ...
            {addGrocOpen && (
              <div style={{ background: "#fff", border: "1px solid #d4c9b8", borderRadius: 12, padding: "16px", marginBottom: 10, display: "flex", flexDirection: "column", gap: 10, boxShadow: "0 4px 16px rgba(0,0,0,0.08)" }}>
                <div style={{ fontSize: 11, color: memberColors[who], fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", display: "flex", alignItems: "center", gap: 6 }}><Avatar name={who} size={16} /> {who} is adding</div>
                <div style={{ display: "flex", gap: 8 }}>
                  <input value={ggItem} onChange={e => setGgItem(e.target.value)} placeholder="Item name e.g. Carrots" style={{ ...IS, flex: 2 }} autoFocus />
                  <input value={ggQty} onChange={e => setGgQty(e.target.value)} placeholder="Qty e.g. 500g" style={{ ...IS, flex: 1 }} />
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <select value={ggCat} onChange={e => setGgCat(e.target.value)} style={{ ...SS, flex: 1 }}>{GROCERY_CATS.map(c => <option key={c}>{c}</option>)}</select>
                  <select value={ggMeal} onChange={e => setGgMeal(e.target.value)} style={{ ...SS, flex: 1 }}>
                    {mealNames.map(mn => <option key={mn} value={mn === "(General)" ? "" : mn}>{mn}</option>)}
                  </select>
                </div>
              ...
              return (
                <div key={cat} style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    {items.map(g => { const mine = g.addedBy === who; const editing = grpId === g.id; return (
                      <div key={g.id}>
                        <div className="row" style={rowBase(editing, g.bought ? "1px solid #7ab87a" : undefined)}>
                          ...
                          <Avatar name={g.addedBy} size={20} />
                          {mine && <><IBtn emoji="✏️" title="Edit" onClick={() => editing ? setGrpId(null) : (setGr({ item: g.item, qty: g.qty || "", category: g.category, forMeal: g.forMeal || "" }), setGrpId(g.id))} /><IBtn emoji="🗑" title="Delete" onClick={() => delGroc(g.id)} /></>}
                        </div>
                        {editing && <EditDrawer>
                          <div style={{ display: "flex", gap: 8 }}>
                          ...
                          <div style={{ display: "flex", gap: 8 }}>
                            ...
                            <select value={gr.forMeal || ""} onChange={e => setGr(v => ({ ...v, forMeal: e.target.value }))} style={{ ...SS, flex: 1 }}>
                              {mealNames.map(mn => <option key={mn} value={mn === "(General)" ? "" : mn}>{mn}</option>)}
                            </select>
                          </div>
            ...
            {filteredGroceries.length === 0 && <div style={{ background: "#fff", border: "1px dashed #d4c9b8", borderRadius: 11, padding: "20px", color: "#b0a090", fontSize: 13, textAlign: "center" }}>Nothing here — tap + Add to start!</div>}
          </div>
        )}
        ...
        {!loading && tab === "Activities" && (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              {[...acts].sort((a, b) => b.votes - a.votes).map(a => { const hv = (a.votedBy || []).includes(who); const mine = a.suggestedBy === who; const editing = eaId === a.id; return (
                <div key={a.id}>
                  <div className="row" style={rowBase(editing, mine ? "1px solid #b8d4b8" : undefined)}>
                    ...
                    {mine && <><IBtn emoji="✏️" title="Edit" onClick={() => editing ? setEaId(null) : (setEa({ name: a.name, day: a.day }), setEaId(a.id))} /><IBtn emoji="🗑" title="Delete" onClick={() => delAct(a.id)} /></>}
                  </div>
                  {editing && <EditDrawer>
            ...
            <p style={{ marginTop: 10, fontSize: 11, color: "#b0a090", textAlign: "center" }}>△/▲ to vote · ✏️ edit your suggestions · 🗑 delete</p>
          </div>
        )}
      </div>
    </div>
